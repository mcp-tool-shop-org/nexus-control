...
- [x] `[all]` No telemetry by default — state it explicitly even if obvious (2026-02-27 — no telemetry, ephemeral data only)

### Default safety posture

- [x] `[cli|mcp|desktop]` SKIP: library only — no dangerous actions, no CLI entry point for kill/delete/restart
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27 — no filesystem writes, audit exports go to caller-specified paths)
- [ ] `[mcp]` SKIP: not an MCP server (library consumed by nexus-router)
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — typed exceptions with deterministic error paths)
- [ ] `[cli]` SKIP: library only, no standalone CLI
- [ ] `[cli]` SKIP: library only, no standalone CLI
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
...
- [ ] `[cli]` SKIP: library only, no standalone CLI
- [ ] `[cli|mcp|desktop]` SKIP: library — logging handled by consumer
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: ARCHITECTURE.md + QUICKSTART.md cover design and usage
